...
- **Cancel requires session-specific URL** — `cancel_meeting` with just the vanity URL returns `session_not_found`. You need the session code path (`/meet/johnanderson/a2tztn`) for cancellation. The session code is embedded in the Google Calendar event description ("Need to change or cancel?" link), not in the `propose_lock` API response. This makes programmatic cancellation harder — you'd need to check the calendar event to find the code. Would be great if `propose_lock` returned the full session URL or if `cancel_meeting` accepted just the `sessionId`.
